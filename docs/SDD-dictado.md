...
| Caso | Cómo | Estado |
|---|---|---|
| (a) dictar a Claude Code en la terminal de WSL | `dictado dictar` (+ `--tecla` para mantener Ctrl derecho); el texto se pega con `^v` en la ventana con el foco | <!-- CASO_A -->**pendiente de la prueba con el humano.** Todo lo de máquina está demostrado (captura, VAD, motor, pegador de Windows leyendo y escribiendo el portapapeles, tecla global conectada desde Windows); lo que falta es la frase dicha por una persona y su confirmación de que apareció donde tenía el cursor. Se dejó corriendo `dictar --tecla` el 2026-09-15 esperando esa frase y no llegó antes de cerrar la rama<!-- /CASO_A --> |
| (b) dictar en cualquier app de Windows | lo mismo: el pegador no sabe qué ventana es | <!-- CASO_B -->**pendiente**, por lo mismo que (a): es la misma prueba con el cursor en otra app<!-- /CASO_B --> |
